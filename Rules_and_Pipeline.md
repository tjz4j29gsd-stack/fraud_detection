...
## 1. A Note on the Confirmed Fraud Dataset (Bias & Limitations)

The transactions in `confirmed_fraud.csv` **are true fraud** (confirmed via customer disputes). However, this dataset is inherently "biased" because it only contains fraud that customers *noticed and reported*. Many other fraudulent transactions likely exist in the 1M rows but were never reported. 

Therefore, we **cannot** assume an unflagged transaction is legitimate. We used this dataset during exploratory data analysis (EDA) to understand the *types* of fraud hitting the platform, but we did not treat it as an absolute ground truth for evaluating our false-positive rates. It was used purely as a directional guide for crafting our rules.
